...
- **設定の永続化**: localStorage(キー `adlib-settings-v1`)にキー・BPM・拍子(カスタム含む)・小節/コード・自動進行・ノリ・サウンド・カウントイン・おまかせ・進行モード・カテゴリフィルタを保存。復元は「DOMに値をセット→changeイベントをdispatch」でハンドラのロジックを再利用する方式(状態変数を直接書かない)。ハンドラ定義より後に `loadSettings()` を呼ぶこと。
- **カウントイン**: ON時は最初の1小節をクリックのみで演奏(クリックOFF設定でも鳴らす)。barLoopを `.start("1m")` で1小節遅らせるだけで、コード/スケール音・進行カウントすべてが後ろにずれる。
- **Wake Lock**: 再生中 `navigator.wakeLock.request("screen")` で画面スリープ防止。visibilitychangeで復帰時に再取得+`Tone.context.resume()`(バックグラウンドでオーディオがsuspendされるため)。
- **サウンドレイヤー**: クリック / コード音 / スケール音 の3つを個別ON/OFF(「コードだけ」「スケールだけ」等が可能)。コード音は小節頭にPolySynthでコードを1小節サステイン(ルートはC3帯、CHORD_IVマップ参照)。スケール音は1小節を8等分してスケール1オクターブ+ルートを駆け上がる。どちらも `Tone.Loop("1m")` のbarLoopで、トグルは毎小節評価なので再生中の切替も即反映。クリックOFF時はガイド音も鳴らさない(拍ドットは常時動作)。
...
- **コード自動進行**: N小節ごと(1/2/4/8)に次のランダムコードへ。barLoop("1m")内の小節カウンタ(barCount/lastChangeBar)で判定し、**抽選(pickNext)は同期実行・画面描画のみTone.Draw遅延**。ON/OFF可。手動「次のコード」ボタンあり。
  - ※旧実装(別Loop+Tone.Draw内でcurrent更新)は、切替小節でコード音が旧コードを鳴らす「1小節遅れ」バグがあった。コード音・スケール音はcurrentを同期で読むため、currentの更新は必ずオーディオコールバック内で同期的に行うこと。
...
- **スケールおまかせ(scaleVariety)**: 各エントリ/進行ステップの `alts` に代替スケール({sr,sc}の配列)を定義し、ONのとき基本スケール+altsから抽選して `currentScale` に保持。表示・譜面・スケール音はすべて `currentScale` を参照(current.sr/scを直接使わないこと)。追加スケール: メジャー/マイナーペンタ(ch空=特徴音なし表示)、ブルース、ドミナントディミニッシュ(H-W)。
- **ジャンル別進行(PROGS)**: ランダム出題に加え、ジャズⅱ-Ⅴ-Ⅰ/マイナーⅱ-Ⅴ-ⅰ/12小節ブルース/ポップスⅠ-Ⅴ-ⅵ-Ⅳ/ボサノバ/ファンクの固定進行をループ。各ステップは {roman,cr,ct,sr,sc,cat,bars,alts} で、bars(小節数)は進行側が持つ(「小節/コード」設定はランダム出題時のみ有効)。コードタイプを拡張: 6/m6/9/m9/maj9/7♭9/13(SUFとCHORD_IVの両方に追加が必要)。バッジは「ジャンル名・ローマ数字 (step/total)」表示。
